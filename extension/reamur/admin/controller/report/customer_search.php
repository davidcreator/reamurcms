 ...
 */
class CustomerSearch extends \Reamur\System\Engine\Controller {
	/**
	 * @return void
	 */
	public function index(): void {
		$this->load->language('extension/reamur/report/customer_search');
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/reamur/report/customer_search', 'user_token=' . $this->session->data['user_token'])
		];

		$data['save'] = $this->url->link('extension/reamur/report/customer_search.save', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/reamur/report/customer_search_form', $data));
	}
	...
	public function save(): void {
		...
		if (!$this->user->hasPermission('modify', 'extension/reamur/report/customer_search')) {
	...
	public function report(): void {
		...
		$this->response->setOutput($this->load->view('extension/reamur/report/customer_search', $data));
	}
	...
	public function getReport(): string {
		...
		$search_total = $this->model_extension_reamur_report_customer->getTotalCustomerSearches($filter_data);

		$results = $this->model_extension_reamur_report_customer->getCustomerSearches($filter_data);
		...
		$data['pagination'] = $this->load->controller('common/pagination', [
			...
			'page'  => $page,
			'limit' => $this->config->get('config_pagination'),
			'url'   => $this->url->link('extension/reamur/report/customer_search.report', 'user_token=' . $this->session->data['user_token'] . '&code=customer_search' . $url . '&page={page}')
		]);
		...
		return $this->load->view('extension/reamur/report/customer_search_list', $data);
	}
}
