...
CREATE TABLE `rms_mooc_instructor` (
  ...
  `mp_user_id` varchar(64) DEFAULT NULL,
  `payout_share` decimal(5,2) NOT NULL DEFAULT 80.00,
...
CREATE TABLE `rms_payment` (
  ...
  CONSTRAINT `rms_payment_customer_fk` FOREIGN KEY (`customer_id`) REFERENCES `rms_customer` (`customer_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `rms_subscription` (
  ...
  CONSTRAINT `rms_subscription_plan_fk` FOREIGN KEY (`plan_id`) REFERENCES `rms_subscription_plan` (`plan_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `rms_subscription_customer_fk` FOREIGN KEY (`customer_id`) REFERENCES `rms_customer` (`customer_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `rms_subscription_payment_fk` FOREIGN KEY (`payment_id`) REFERENCES `rms_payment` (`payment_id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `rms_blog_post` ADD COLUMN `is_premium` tinyint(1) NOT NULL DEFAULT 0 AFTER `status`, ADD COLUMN `owner_id` int(11) DEFAULT NULL AFTER `is_premium`, ADD COLUMN `payout_share` decimal(5,2) NOT NULL DEFAULT 80.00 AFTER `owner_id`;
ALTER TABLE `rms_landpage` ADD COLUMN `is_premium` tinyint(1) NOT NULL DEFAULT 0 AFTER `status`, ADD COLUMN `owner_id` int(11) DEFAULT NULL AFTER `is_premium`, ADD COLUMN `payout_share` decimal(5,2) NOT NULL DEFAULT 80.00 AFTER `owner_id`;


ALTER TABLE `rms_blog_post` ADD COLUMN `price` decimal(10,2) NOT NULL DEFAULT 0.00 AFTER `payout_share`;
ALTER TABLE `rms_landpage` ADD COLUMN `price` decimal(10,2) NOT NULL DEFAULT 0.00 AFTER `payout_share`;
