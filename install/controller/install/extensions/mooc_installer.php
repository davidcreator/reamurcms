...
/**
 * Instala as tabelas e configurações iniciais da extensão de MOOC.
 *
 * Rota: install/extensions/mooc_installer
 * Método recomendado: POST (recebe credenciais de banco) ou usa config.php se presente.
 */
class MoocInstaller extends Controller {
	/**
	...
	public function index(): void {
		...
		try {
			...
			$executed = $this->model_extensions_mooc_install->install($dbData);

			$this->respond([
				'success'  => true,
				'executed' => $executed,
				'message'  => 'Extensão de MOOC instalada com sucesso.'
			]);
		} catch (Throwable $e) {
			$this->respond([
				'success' => false,
