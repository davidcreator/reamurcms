...
/**
 * Instala as tabelas e configurações iniciais da extensão de Landing Pages.
 *
 * Rota: install/extensions/landpage_installer
 * Método recomendado: POST (recebe credenciais de banco) ou usa config.php se presente.
 */
class LandpageInstaller extends Controller {
	/**
	...
	public function index(): void {
		...
		try {
			...
			$executed = $this->model_extensions_landpage_install->install($dbData);

			$this->respond([
				'success'  => true,
				'executed' => $executed,
				'message'  => 'Extensão de landing pages instalada com sucesso.'
			]);
		} catch (Throwable $e) {
			$this->respond([
				'success' => false,
