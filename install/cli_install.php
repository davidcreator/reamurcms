<?php
// Command line tool for installing reamurcms
// Original Author: David L. Almeida <contato@davidalmeida.xyz> <davidalmeida.designer@gmail.com>
// Updated and maintained by ReamurCMS
...
//                               --password    password
//                               --http_server http://localhost/reamurcms/
//                               --db_driver   mysqli
//                               --db_hostname localhost
//                               --db_username root
//                               --db_password pass
//                               --db_database reamurcms
//								 --db_port     3306
//                               --db_prefix   rms_
//
// Example:
//
// php c://xampp/htdocs/reamurcms-master/upload/install/cli_install.php install --username admin --password mexico --email email@example.com --http_server http://localhost/reamurcms-master/upload/ --db_driver mysqli --db_hostname localhost --db_username root --db_database reamurcms-master --db_port 3306 --db_prefix rms_
//
...
// DIR
define('DIR_REAMUR', str_replace('\\', '/', realpath(dirname(__FILE__) . '/../')) . '/');
define('DIR_APPLICATION', DIR_REAMUR . 'install/');
define('DIR_EXTENSION', DIR_REAMUR . 'extension/');
define('DIR_SYSTEM', DIR_REAMUR . 'system/');
define('DIR_IMAGE', DIR_REAMUR . 'image/');
...
// Request
$registry->set('request', new \Reamur\System\Library\Request());
...
class CliInstall extends \Reamur\System\Engine\Controller {
	...
	public function install($argv): string {
		...
		if (version_compare(phpversion(), '8.0.0', '<')) {
			$error .= 'ERROR: You need to use PHP8+ or above for ReamurCMS to work!' . "\n";
		}
		...
		if (ini_get('session.auto_start')) {
			$error .= 'ERROR: ReamurCMS will not work with session.auto_start enabled!' . "\n";
		}

		if (!extension_loaded('mysqli')) {
			$error .= 'ERROR: MySQLi extension needs to be loaded for ReamurCMS to work!' . "\n";
		}

		if (!extension_loaded('gd')) {
			$error .= 'ERROR: GD extension needs to be loaded for ReamurCMS to work!' . "\n";
		}

		if (!extension_loaded('curl')) {
			$error .= 'ERROR: CURL extension needs to be loaded for ReamurCMS to work!' . "\n";
		}

		if (!function_exists('openssl_encrypt')) {
			$error .= 'ERROR: OpenSSL extension needs to be loaded for ReamurCMS to work!' . "\n";
		}

		if (!extension_loaded('zlib')) {
			$error .= 'ERROR: ZLIB extension needs to be loaded for ReamurCMS to work!' . "\n";
		}

		if (!is_file(DIR_REAMUR . 'config.php')) {
			$error .= 'ERROR: config.php does not exist. You need to rename config-dist.php to config.php!' . "\n";
		} elseif (!is_writable(DIR_REAMUR . 'config.php')) {
			$error .= 'ERROR: config.php needs to be writable for ReamurCMS to be installed!' . "\n";
		} elseif (!is_file(DIR_REAMUR . 'admin/config.php')) {
			$error .= 'ERROR: admin/config.php does not exist. You need to rename admin/config-dist.php to admin/config.php!' . "\n";
		} elseif (!is_writable(DIR_REAMUR . 'admin/config.php')) {
			$error .= 'ERROR: admin/config.php needs to be writable for ReamurCMS to be installed!' . "\n";
		}
		...
		if ((rms_strlen($option['username']) < 3) || (rms_strlen($option['username']) > 20)) {
			$error .= 'ERROR: Username must be between 3 and 20 characters!' . "\n";
		}

		if ((rms_strlen($option['email']) > 96) || !filter_var($option['email'], FILTER_VALIDATE_EMAIL)) {
		...
		if ($option['db_password']) {
			...
			if ((rms_strlen($password) < 5) || (rms_strlen($password) > 20)) {
		...
		$file = DIR_APPLICATION . 'reamurcms.sql';
		...
		try {
			// Database
			$db = new \Reamur\System\Library\DB($db_driver, $db_hostname, $db_username, $db_password, $db_database, $db_port);
		} catch (\Exception $e) {
		...
		if ($lines) {
			...
			foreach ($lines as $line) {
				...
				if (substr($line, -2) == ');') {
					$db->query(str_replace("INSERT INTO `rms_", "INSERT INTO `" . $db_prefix, $sql));
			...
			$db->query("INSERT INTO `" . $db_prefix . "setting` SET `code` = 'config', `key` = 'config_encryption', `value` = '" . $db->escape(rms_token(1024)) . "'");

			$db->query("INSERT INTO `" . $db_prefix . "api` SET `username` = 'Default', `key` = '" . $db->escape(rms_token(256)) . "', `status` = 1, `date_added` = NOW(), `date_modified` = NOW()");
		...
		$output .= '// DIR' . "\n";
		$output .= 'define(\'DIR_REAMUR\', \'' . DIR_REAMUR . '\');' . "\n";
		$output .= 'define(\'DIR_APPLICATION\', DIR_REAMUR . \'catalog/\');' . "\n";
		$output .= 'define(\'DIR_SYSTEM\', DIR_REAMUR . \'system/\');' . "\n";
		$output .= 'define(\'DIR_EXTENSION\', DIR_REAMUR . \'extension/\');' . "\n";
		$output .= 'define(\'DIR_IMAGE\', DIR_REAMUR . \'image/\');' . "\n";
		...
		$file = fopen(DIR_REAMUR . 'config.php', 'w');
		...
		$output .= '// DIR' . "\n";
		$output .= 'define(\'DIR_REAMUR\', \'' . DIR_REAMUR . '\');' . "\n";
		$output .= 'define(\'DIR_APPLICATION\', DIR_REAMUR . \'admin/\');' . "\n";
		$output .= 'define(\'DIR_SYSTEM\', DIR_REAMUR . \'system/\');' . "\n";
		$output .= 'define(\'DIR_EXTENSION\', DIR_REAMUR . \'extension/\');' . "\n";
		$output .= 'define(\'DIR_IMAGE\', DIR_REAMUR . \'image/\');' . "\n";
		$output .= 'define(\'DIR_STORAGE\', DIR_SYSTEM . \'storage/\');' . "\n";
		$output .= 'define(\'DIR_CATALOG\', DIR_REAMUR . \'catalog/\');' . "\n";
		$output .= 'define(\'DIR_LANGUAGE\', DIR_APPLICATION . \'language/\');' . "\n";
		$output .= 'define(\'DIR_TEMPLATE\', DIR_APPLICATION . \'view/template/\');' . "\n";
		$output .= 'define(\'DIR_CONFIG\', DIR_SYSTEM . \'config/\');' . "\n";
		$output .= 'define(\'DIR_CACHE\', DIR_STORAGE . \'cache/\');' . "\n";
		...
		$output .= '// Reamur API' . "\n";
		$output .= 'define(\'REAMUR_SERVER\', \'https://www.reamurcms.com/\');';

		$file = fopen(DIR_REAMUR . 'admin/config.php', 'w');
		...
		$output  = 'SUCCESS! ReamurCMS successfully installed on your server' . "\n";
