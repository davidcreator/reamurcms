 ...
 * Class Register
 *
 * @package Reamur\Catalog\Controller\Checkout
 */
class Register extends \Reamur\System\Engine\Controller {
	/**
	 * @return string
	 */
	...
	public function save(): void {
		...
		if (!$json) {
			...
			if ((rms_strlen($this->request->post['firstname']) < 1) || (rms_strlen($this->request->post['firstname']) > 32)) {
				$json['error']['firstname'] = $this->language->get('error_firstname');
			}

			if ((rms_strlen($this->request->post['lastname']) < 1) || (rms_strlen($this->request->post['lastname']) > 32)) {
				$json['error']['lastname'] = $this->language->get('error_lastname');
			}

			if ((rms_strlen($this->request->post['email']) > 96) || !filter_var($this->request->post['email'], FILTER_VALIDATE_EMAIL)) {
			...
			if ($this->config->get('config_telephone_required') && (rms_strlen($this->request->post['telephone']) < 3) || (rms_strlen($this->request->post['telephone']) > 32)) {
			...
			if ($this->config->get('config_checkout_payment_address')) {
				if ((rms_strlen($this->request->post['payment_address_1']) < 3) || (rms_strlen($this->request->post['payment_address_1']) > 128)) {
					$json['error']['payment_address_1'] = $this->language->get('error_address_1');
				}

				if ((rms_strlen($this->request->post['payment_city']) < 2) || (rms_strlen($this->request->post['payment_city']) > 128)) {
				...
				if ($payment_country_info && $payment_country_info['postcode_required'] && (rms_strlen($this->request->post['payment_postcode']) < 2 || rms_strlen($this->request->post['payment_postcode']) > 10)) {
			...
			if ($this->cart->hasShipping() && !$this->request->post['address_match']) {
				...
				if ($this->config->get('config_checkout_payment_address')) {
					if ((rms_strlen($this->request->post['shipping_firstname']) < 1) || (rms_strlen($this->request->post['shipping_firstname']) > 32)) {
						$json['error']['shipping_firstname'] = $this->language->get('error_firstname');
					}

					if ((rms_strlen($this->request->post['shipping_lastname']) < 1) || (rms_strlen($this->request->post['shipping_lastname']) > 32)) {
						$json['error']['shipping_lastname'] = $this->language->get('error_lastname');
					}
				}

				if ((rms_strlen($this->request->post['shipping_address_1']) < 3) || (rms_strlen($this->request->post['shipping_address_1']) > 128)) {
					$json['error']['shipping_address_1'] = $this->language->get('error_address_1');
				}

				if ((rms_strlen($this->request->post['shipping_city']) < 2) || (rms_strlen($this->request->post['shipping_city']) > 128)) {
				...
				if ($shipping_country_info && $shipping_country_info['postcode_required'] && (rms_strlen($this->request->post['shipping_postcode']) < 2 || rms_strlen($this->request->post['shipping_postcode']) > 10)) {
			...
			if ($this->request->post['account'] && (rms_strlen(html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8')) < 4) || (rms_strlen(html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8')) > 40)) {
		...
		if (!$json) {
			...
			if (!$customer_group_info['approval']) {
				if ($this->request->post['account']) {
					...
					$this->session->data['customer_token'] = rms_token(26);
