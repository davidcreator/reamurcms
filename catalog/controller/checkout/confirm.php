 ...
 */
class Confirm extends \Reamur\System\Engine\Controller {
	/**
	 * @return string
	 */
	public function index(): string {
		...
		foreach ($products as $product) {
			if ($product['option']) {
				foreach ($product['option'] as $key => $option) {
					$product['option'][$key]['value'] = (rms_strlen($option['value']) > 20 ? rms_substr($option['value'], 0, 20) . '..' : $option['value']);
				}
			}
		...
		if (isset($this->session->data['payment_method'])) {
			$code = rms_substr($this->session->data['payment_method']['code'], 0, strpos($this->session->data['payment_method']['code'], '.'));
		} else {
			$code = '';
		}
