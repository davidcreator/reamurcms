<?php
namespace Reamur\Catalog\Controller\Product;
/**
 * Class Manufacturer
 *
 * @package Reamur\Catalog\Controller\Product
 */
class Manufacturer extends \Reamur\System\Engine\Controller {
	/**
	 * @return void
	 */
	public function index(): void {
		...
		foreach ($results as $result) {
			if (is_numeric(rms_substr($result['name'], 0, 1))) {
				$key = '0 - 9';
			} else {
				$key = rms_substr(rms_strtoupper($result['name']), 0, 1);
			}
	 ...
	 * @return void
	 */
	public function info(): \Reamur\System\Engine\Action|null {
		$this->load->language('product/manufacturer');
		...
		if ($manufacturer_info) {
			...
			foreach ($results as $result) {
				...
				$product_data = [
					...
					'thumb'       => $image,
					'name'        => $result['name'],
					'description' => rms_substr(trim(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, $this->config->get('config_product_description_length')) . '..',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
