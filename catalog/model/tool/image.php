<?php
namespace Reamur\Catalog\Model\Tool;
/**
 * Class Image
 *
 * @package Reamur\Catalog\Model\Tool
 */
class Image extends \Reamur\System\Engine\Model {
	/**
	...
	public function resize(string $filename, int $width, int $height, string $default = ''): string {
		...
		$image_old = $filename;
		$image_new = 'cache/' . rms_substr($filename, 0, rms_strrpos($filename, '.')) . '-' . (int)$width . 'x' . (int)$height . '.' . $extension;
		...
		if (!is_file(DIR_IMAGE . $image_new) || (filemtime(DIR_IMAGE . $image_old) > filemtime(DIR_IMAGE . $image_new))) {
			...
			if ($width_orig != $width || $height_orig != $height) {
				$image = new \Reamur\System\Library\Image(DIR_IMAGE . $image_old);
