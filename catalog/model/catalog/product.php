<?php
namespace Reamur\Catalog\Model\Catalog;
/**
 * Class Product
 *
 * @package Reamur\Catalog\Model\Catalog
 */
class Product extends \Reamur\System\Engine\Model {
	...
	public function getProducts(array $data = []): array {
		...
		if (!empty($data['filter_search']) || !empty($data['filter_tag'])) {
			...
			if (!empty($data['filter_search'])) {
				$sql .= " OR LCASE(`p`.`model`) = '" . $this->db->escape(rms_strtolower($data['filter_search'])) . "'";
				$sql .= " OR LCASE(`p`.`sku`) = '" . $this->db->escape(rms_strtolower($data['filter_search'])) . "'";
				$sql .= " OR LCASE(`p`.`upc`) = '" . $this->db->escape(rms_strtolower($data['filter_search'])) . "'";
				$sql .= " OR LCASE(`p`.`ean`) = '" . $this->db->escape(rms_strtolower($data['filter_search'])) . "'";
				$sql .= " OR LCASE(`p`.`jan`) = '" . $this->db->escape(rms_strtolower($data['filter_search'])) . "'";
				$sql .= " OR LCASE(`p`.`isbn`) = '" . $this->db->escape(rms_strtolower($data['filter_search'])) . "'";
				$sql .= " OR LCASE(`p`.`mpn`) = '" . $this->db->escape(rms_strtolower($data['filter_search'])) . "'";
			}
