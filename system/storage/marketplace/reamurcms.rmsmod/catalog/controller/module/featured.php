...
class Featured extends \Reamur\System\Engine\Controller {
	public function index(array $setting): string {
		...
		if (!empty($setting['product'])) {
			...
			foreach ($products as $product) {
				...
				$product_data = [
					...
					'thumb'       => $image,
					'name'        => $product['name'],
					'description' => rms_substr(trim(strip_tags(html_entity_decode($product['description'], ENT_QUOTES, 'UTF-8'))), 0, $this->config->get('config_product_description_length')) . '..',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
		...
		if ($data['products']) {
			return $this->load->view('extension/reamur/module/featured', $data);
		} else {
			return '';
		}
	}
