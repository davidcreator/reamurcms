<?php
namespace Reamur\Catalog\Controller\Extension\OcPaymentExample\Payment;
class CreditCard extends \Reamur\System\Engine\Controller {
	public function index(): string {
		...
		if (isset($this->session->data['payment_method'])) {
			...
			if ($this->session->data['payment_method']['code'] == 'credit_card.credit_card') {
				return $this->load->view('extension/rms_payment_example/payment/credit_card', $data);
			} else {
				return $this->load->view('extension/rms_payment_example/payment/stored', $data);
			}
		}
		...
		if (!$json) {
			...
			*/
			...
			if ($this->customer->isLogged() && ($this->request->post['store'] || $this->cart->hasSubscription())) {
				...
				$this->model_extension_rms_payment_example_payment_credit_card->addCreditCard($this->customer->getId(), $credit_card_data);
			}
	...
	public function stored(): void {
		...
		$credit_card_info = $this->model_extension_rms_payment_example_payment_credit_card->getCreditCard($this->customer->getId(), $credit_card_id);
		...
		if (!$json) {
			...
			// Charge
			$response = $this->model_extension_rms_payment_example_payment_credit_card->charge($this->customer->getId(), $this->session->data['order_id'], $order_info['total'], $credit_card_id);
	...
	public function delete(): void {
		...
		if (!$json) {
			$this->model_extension_rms_payment_example_payment_credit_card->deleteCreditCard($this->customer->getId(), $credit_card_id);
