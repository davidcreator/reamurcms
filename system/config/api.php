<?php
// Site
$_['site_url']                 = HTTP_SERVER;
//$_['site_ssl']               = HTTPS_SERVER;

// Url
$_['url_autostart']            = false;

// Security
$_['api_secret_key']           = 'reamurcms';
$_['api_cache_expire']         = 60;
$_['api_request_per_minute']   = 30;

// Igrore middleawares in
$_['ignored_routters']         = [
                                       'credentials/access_token',
                                       'credentials/refresh_token',
                                       'common/documentation',
                                 ];

// Log
$_['error_filename']           = sprintf('api-%d-%s-%s.log',
                                        date('Y'),
                                                date('m'),
                                                date('d')
                                        );

// Database
$_['db_autostart']             = true;
$_['db_driver']                = DB_DRIVER; // mysqli, pdo or pgsql
$_['db_hostname']              = DB_HOSTNAME;
$_['db_username']              = DB_USERNAME;
$_['db_password']              = DB_PASSWORD;
$_['db_database']              = DB_DATABASE;
$_['db_port']                  = DB_PORT;
$_['db_list_per_page']         = 50;

// Session
$_['session_autostart']        = true;
$_['session_engine']           = 'db';
$_['session_name']             = 'RMSSESSION';

// Template
$_['image_max_filesize']       = '2 * 1024 * 1024'; // 2mb in bytes
$_['image_dir_download']       = DIR_IMAGE . 'catalog/api';
$_['image_download_overwrite'] = false;
$_['image_mimetypes']          = [
                                  'image/jpeg',
                                  'image/pjpeg',
                                  'image/png',
                                  'image/x-png',
                                  'image/svg',
                                  'image/svg+xml',
                                  'image/webp',
                                  'image/gif',
                                ];

// Autolead Libraries
$_['library_autoload']        = [];

// Actions
$_['action_pre_action']        = [
                                       'middlewares/response_time/before',
                                       'startup/startup',
                                       'startup/event',
                                       'startup/seo_url',
                                       'startup/login',
                                  ];

// Action Events
$_['action_event']             = [
	                                    'controller/*/before' => [
		                                    'middlewares/logs/before',
		                                    'middlewares/status/before',
		                                    'middlewares/access_token_expired/before',
		                                    'middlewares/check_request_method/before',
		                                    'middlewares/ip/before',
		                                    'middlewares/rate_limit/before',
                                      ],
	                                    'controller/*/after' => [
		                                    'middlewares/logs/after',
	                                    ]
                                  ];

// Webhook Events
$_['action_webhook']          = [
                                      'model/catalog/product/add/after'           => [
                                        'catalog/product/add'
                                      ],
                                      'model/catalog/product/updates/after'       => [
                                        'catalog/product/edit'
                                      ],
                                      'model/catalog/product/updateStock/after'   => [
                                        'catalog/product/edit'
                                      ],
                                      'model/catalog/product/deleteProduct/after' => [
                                        'catalog/product/delete'
                                      ],
                                ];