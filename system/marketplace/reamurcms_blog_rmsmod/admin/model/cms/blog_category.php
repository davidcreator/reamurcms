...
class BlogCategory extends \Reamur\System\Engine\Model {
    ...
    public function delete(int $category_id): void {
        ...
        // reset children parent to root
        $this->db->query("UPDATE `" . DB_PREFIX . "blog_category` SET parent_id = 0 WHERE parent_id = '" . (int)$category_id . "'");
    }
    ...
    public function getTotalCategories(array $data = []): int {
        $sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "blog_category` WHERE 1";
        if (isset($data['status'])) {
            $sql .= " AND status = '" . (int)$data['status'] . "'";
        }
    ...
    public function getCategoryOptions(): array {
        $rows = $this->db->query("SELECT category_id, name FROM `" . DB_PREFIX . "blog_category` ORDER BY name")->rows;
        $options = [];
        foreach ($rows as $row) {
