...
class Landpage extends \Reamur\System\Engine\Controller {
    ...
    public function info(): void {
        ...
        if (!empty($page['is_premium'])) {
            ...
            $paid = $this->customer->isLogged() ? $this->model_checkout_payment->getByContext('landpage', (int)$page['page_id'], $this->customer->getId()) : [];
            $access = !$page['price'] || $activeSub || $paid;
            if (!$access) {
                $page['html'] = '<p>' . ($this->language->get('text_payment_required') ?? 'Conteúdo reservado a assinantes.') . '</p>';
                $data['purchase_link'] = $this->url->link('checkout/content_payment.checkout', 'context=landpage&ref=' . (int)$page['page_id']);
            }
        }
