...
class Blog extends \Reamur\System\Engine\Model {
    ...
    public function getPost(int $post_id): array {
        $query = $this->db->query("SELECT *, is_premium, owner_id, payout_share, price FROM `" . DB_PREFIX . "blog_post` WHERE post_id = '" . (int)$post_id . "' AND status = 'published'");
        return $query->row ?? [];
    }

    public function getPostBySlug(string $slug): array {
        $query = $this->db->query("SELECT *, is_premium, owner_id, payout_share, price FROM `" . DB_PREFIX . "blog_post` WHERE slug = '" . $this->db->escape($slug) . "' AND status = 'published'");
        return $query->row ?? [];
    }
