...
class MoocEnrollment extends \Reamur\System\Engine\Model {
    ...
    public function updateLessonProgress(int $course_id, int $customer_id, int $lesson_id, string $status, int $time_spent_seconds = 0, $score = null): void {
        ...
        $existing = $this->db->query("SELECT progress_id, time_spent_seconds, status FROM `" . DB_PREFIX . "mooc_progress` WHERE enrollment_id = '" . $enrollment_id . "' AND lesson_id = '" . (int)$lesson_id . "'")->row;
        ...
        if ($score !== null && $status === 'completed') {
            $lesson = $this->db->query("SELECT title FROM `" . DB_PREFIX . "mooc_lesson` WHERE lesson_id = '" . (int)$lesson_id . "'")->row;
            $this->load->model('cms/mooc_notification');
            $title = 'Nota publicada';
            $message = 'Sua nota para a aula ' . ($lesson['title'] ?? '') . ' foi registrada.';
            $url = '/index.php?route=cms/mooc.lesson&course_id=' . (int)$course_id . '&lesson_id=' . (int)$lesson_id;
            $this->model_cms_mooc_notification->addNotification($customer_id, $title, $message, $url, 'system', $course_id, $lesson_id);
        }
        ...
        if ($status === 'completed' && !$was_completed) {
            ...
            $this->model_cms_mooc_gamification->completeLesson($customer_id, $course_id, $lesson_id);
        }

        $updated = $this->getEnrollment($course_id, $customer_id);
        $was_course_completed = ($enrollment['progress_percent'] ?? 0) >= 100;
        if (!$was_course_completed && (int)($updated['progress_percent'] ?? 0) >= 100) {
            $this->load->model('cms/mooc_gamification');
            $this->model_cms_mooc_gamification->completeCourse($customer_id, $course_id);
        }
    }
    ...
    private function ensureCertificate(int $enrollment_id): void {
        ...
        $enrollment = $this->db->query("SELECT course_id, customer_id FROM `" . DB_PREFIX . "mooc_enrollment` WHERE enrollment_id = '" . (int)$enrollment_id . "'")->row;
        ...
        if ($enrollment) {
            $this->load->model('cms/mooc_notification');
            $url = '/index.php?route=cms/mooc.certificate&course_id=' . (int)$enrollment['course_id'];
            $title = 'Certificado disponível';
            $message = 'Seu certificado do curso está pronto.';
            $this->model_cms_mooc_notification->addNotification((int)$enrollment['customer_id'], $title, $message, $url, 'system', (int)$enrollment['course_id']);
        }
    }
