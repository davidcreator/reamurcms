...
class MoocLesson extends \Reamur\System\Engine\Controller {
    ...
    public function form(): void {
        ...
        if ($this->request->server['REQUEST_METHOD'] === 'POST' && $this->validateForm()) {
            ...
            if ($is_new && !empty($payload['course_id'])) {
                $this->load->model('cms/mooc_notification');
                $title = sprintf($this->language->get('text_new_lesson_notification') ?? 'Nova aula: %s', $payload['title'] ?? '');
                $message = $payload['summary'] ?? '';
                $url = '/index.php?route=cms/mooc.info&course_id=' . (int)$payload['course_id'];
                $this->model_cms_mooc_notification->notifyCourseEnrollees((int)$payload['course_id'], $title, $message, $url, 'system', $lesson_id);
            }
