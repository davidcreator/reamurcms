...
class MoocInstructor extends \Reamur\System\Engine\Model {
    ...
    private function ensureColumns(): void {
        ...
        $col2 = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "mooc_instructor` LIKE 'stripe_account_id'")->num_rows;
        if (!$col2) {
            $this->db->query("ALTER TABLE `" . DB_PREFIX . "mooc_instructor` ADD `stripe_account_id` varchar(64) DEFAULT NULL AFTER `website`, ADD `mp_user_id` varchar(64) DEFAULT NULL AFTER `stripe_account_id`, ADD `payout_share` decimal(5,2) NOT NULL DEFAULT 80.00 AFTER `mp_user_id`");
        }
    }
    ...
    public function add(array $data): int {
        $this->db->query("INSERT INTO `" . DB_PREFIX . "mooc_instructor` SET
            ...
            stripe_account_id = '" . $this->db->escape((string)($data['stripe_account_id'] ?? '')) . "',
            mp_user_id = '" . $this->db->escape((string)($data['mp_user_id'] ?? '')) . "',
