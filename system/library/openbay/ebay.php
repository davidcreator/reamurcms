...
final class Ebay {
	...
	public function call($call, array $post = null, array $options = array(), $content_type = 'json', $status_override = false) {
		if ($this->config->get('ebay_status') == 1 || $status_override == true) {
			...
			$data = array('token' => $this->token, 'secret' => $this->secret, 'server' => $this->server, 'domain' => $domain, 'openbay_version' => (int)$this->config->get('feed_openbaypro_version'), 'reamur_version' => VERSION, 'data' => $post, 'content_type' => $content_type, 'language' => $this->config->get('feed_openbaypro_language'));
			...
			$defaults = array(
				...
				CURLOPT_USERAGENT       => "OpenBay Pro for eBay/Reamur",
	...
	public function putStockUpdateBulk($product_id_array, $end_inactive = false) {
		...
		// Get the active ReamurCMS items that were linked to eBay If they have stock now, relist them.
		$ended_data = $this->getEndedListingArray();

		/**
		 * Get the active ReamurCMS items that are also linked
