...
class Upgrade extends \Reamur\System\Engine\Controller {
	...
	public function index(): void {
		...
		$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/upgrade');
	...
	public function download(): void {
		...
		$file = DIR_DOWNLOAD . 'reamur-' . $version . '.zip';

		$handle = fopen($file, 'w');

		set_time_limit(0);

		$curl = curl_init('https://github.com/reamurcms/reamurcms/archive/' . $version . '.zip');

		curl_setopt($curl, CURLOPT_USERAGENT, 'ReamurCMS ' . VERSION);
		curl_setopt($curl, CURLOPT_FOLLOWLOCATION, 1);
		curl_setopt($curl, CURLOPT_FORBID_REUSE, 1);
		curl_setopt($curl, CURLOPT_FRESH_CONNECT, 1);
	...
	public function install(): void {
		...
		if (!$json) {
			...
			if ($zip->open($file, \ZipArchive::RDONLY)) {
					...
					if (substr($source, 0, strlen($remove)) == $remove) {
						...
						if (substr($destination, 0, 8) == 'install/') {
							...
							foreach ($directories as $directory) {
								...
								if (!is_dir(DIR_REAMUR . $path) && !@mkdir(DIR_REAMUR . $path, 0777)) {
							...
							if (substr($destination, -1) != '/') {
								if (is_file(DIR_REAMUR . $destination)) {
									unlink(DIR_REAMUR . $destination);
								}

								if (!copy('zip://' . $file . '#' . $source, DIR_REAMUR . $destination)) {
				...
				$json['next'] = HTTP_CATALOG . 'install/index.php?route=upgrade/upgrade_1&version=' . $version . '&admin=' . rtrim(substr(DIR_APPLICATION, strlen(DIR_REAMUR), -1));
			} else {
