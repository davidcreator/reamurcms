 ...
 */
class Download extends \Reamur\System\Engine\Controller {
	/**
	 * @return void
	 */
	...
	public function save(): void {
		...
		foreach ($this->request->post['download_description'] as $language_id => $value) {
			if ((rms_strlen(trim($value['name'])) < 3) || (rms_strlen($value['name']) > 64)) {
				$json['error']['name_' . $language_id] = $this->language->get('error_name');
			}
		}

		if ((rms_strlen($this->request->post['filename']) < 3) || (rms_strlen($this->request->post['filename']) > 128)) {
		...
		if ((rms_strlen($this->request->post['mask']) < 3) || (rms_strlen($this->request->post['mask']) > 128)) {
	...
	public function upload(): void {
		...
		if (!$json) {
			...
			if ((rms_strlen($filename) < 3) || (rms_strlen($filename) > 128)) {
