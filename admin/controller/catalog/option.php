 ...
 */
class Option extends \Reamur\System\Engine\Controller {
	/**
	 * @return void
	 */
	...
	public function save(): void {
		...
		foreach ($this->request->post['option_description'] as $language_id => $value) {
			if ((rms_strlen(trim($value['name'])) < 1) || (rms_strlen($value['name']) > 128)) {
		...
		if (isset($this->request->post['option_value'])) {
			foreach ($this->request->post['option_value'] as $option_value_id => $option_value) {
				foreach ($option_value['option_value_description'] as $language_id => $option_value_description) {
					if ((rms_strlen(trim($option_value_description['name'])) < 1) || (rms_strlen($option_value_description['name']) > 128)) {
