<?php
namespace Reamur\Admin\Controller\Catalog;
/**
 * Class Product 
 * @package Reamur\Admin\Controller\Catalog
 */
class Product extends \Reamur\System\Engine\Controller {
	...
	public function index(): void {
		...
		$url = '';
		foreach ([
			'filter_name', 'filter_model', 'filter_price', 'filter_quantity', 'filter_status', 'sort', 'order', 'page'
		] as $param) {
			if (isset($this->request->get[$param])) {
				$value = $this->request->get[$param];
				if (in_array($param, ['filter_name', 'filter_model'])) {
	...
	protected function getList(): string {
		...
		$url = '';
		foreach ([
			'filter_name', 'filter_model', 'filter_price', 'filter_quantity', 'filter_status', 'page'
		] as $param) {
			if (isset($this->request->get[$param])) {
				$value = $this->request->get[$param];
				if (in_array($param, ['filter_name', 'filter_model'])) {
	...
	public function save(): void {
		...
		foreach ($this->request->post['product_description'] as $language_id => $value) {
			if ((rms_strlen(trim($value['name'])) < 1) || (rms_strlen($value['name']) > 255)) {
				$json['error']['name_' . $language_id] = $this->language->get('error_name');
			}

			if ((rms_strlen(trim($value['meta_title'])) < 1) || (rms_strlen($value['meta_title']) > 255)) {
		...
		if ($this->request->post['product_seo_url']) {
			...
			foreach ($this->request->post['product_seo_url'] as $store_id => $language) {
				foreach ($language as $language_id => $keyword) {
					if ((rms_strlen(trim($keyword)) < 1) || (rms_strlen($keyword) > 64)) {
