...
class Order extends \Reamur\System\Engine\Controller {
	...
	public function info(): void {
		...
		if (isset($this->session->data['api_session'])) {
			$session = new \Reamur\System\Library\Session($this->config->get('session_engine'), $this->registry);
		...
		if (!empty($order_info['payment_method']['code'])) {
			if (isset($order_info['payment_method']['code'])) {
				$code = rms_substr($order_info['payment_method']['code'], 0, strpos($order_info['payment_method']['code'], '.'));
			} else {
				$code = '';
			}
	...
	// Method to call the store catalog API and return a response.
