...
class Voucher extends \Reamur\System\Engine\Controller {
	...
	public function save(): void {
		...
		if ((rms_strlen($this->request->post['code']) < 3) || (rms_strlen($this->request->post['code']) > 10)) {
		...
		if ((rms_strlen($this->request->post['to_name']) < 1) || (rms_strlen($this->request->post['to_name']) > 64)) {
			$json['error']['to_name'] = $this->language->get('error_to_name');
		}

		if ((rms_strlen($this->request->post['to_email']) > 96) || !filter_var($this->request->post['to_email'], FILTER_VALIDATE_EMAIL)) {
			$json['error']['to_email'] = $this->language->get('error_email');
		}

		if ((rms_strlen($this->request->post['from_name']) < 1) || (rms_strlen($this->request->post['from_name']) > 64)) {
			$json['error']['from_name'] = $this->language->get('error_from_name');
		}

		if ((rms_strlen($this->request->post['from_email']) > 96) || !filter_var($this->request->post['from_email'], FILTER_VALIDATE_EMAIL)) {
