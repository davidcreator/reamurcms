<?php
namespace Reamur\Admin\Controller\Customer;
/**
 * Class Customer
 *
 * @package Reamur\Admin\Controller\Customer
 */
class Customer extends \Reamur\System\Engine\Controller {
	...
	public function save(): void {
		...
		if ($this->request->post['password'] || (!isset($this->request->post['customer_id']))) {
			if ((rms_strlen(html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8')) < 4) || (rms_strlen(html_entity_decode($this->request->post['password'], ENT_QUOTES, 'UTF-8')) > 40)) {
		...
		if (isset($this->request->post['address'])) {
			foreach ($this->request->post['address'] as $key => $value) {
				if ((rms_strlen($value['firstname']) < 1) || (rms_strlen($value['firstname']) > 32)) {
					$json['error']['address_' . $key . '_firstname'] = $this->language->get('error_firstname');
				}

				if ((rms_strlen($value['lastname']) < 1) || (rms_strlen($value['lastname']) > 32)) {
					$json['error']['address_' . $key . '_lastname'] = $this->language->get('error_lastname');
				}

				if ((rms_strlen($value['address_1']) < 3) || (rms_strlen($value['address_1']) > 128)) {
					$json['error']['address_' . $key . '_address_1'] = $this->language->get('error_address_1');
				}

				if ((rms_strlen($value['city']) < 2) || (rms_strlen($value['city']) > 128)) {
				...
				if (!isset($value['country_id']) || $value['country_id'] == '') {
					...
					if ($country_info && $country_info['postcode_required'] && (rms_strlen($value['postcode']) < 2 || rms_strlen($value['postcode']) > 10)) {
