 ...
 */
class Security extends \Reamur\System\Engine\Controller {
	/**
	 * @return string
	 */
	public function index(): string {
		...
		if (is_dir(DIR_REAMUR . 'install/')) {
			$data['install'] = DIR_REAMUR . 'install/';
		} else {
			$data['install'] = '';
		}
		...
		if (DIR_STORAGE == DIR_SYSTEM . 'storage/') {
			...
			// Fix: Handle missing DOCUMENT_ROOT
			$document_root = $this->getDocumentRoot();
			$data['document_root'] = str_replace('\\', '/', realpath($document_root . '/../')) . '/';
		...
		if (DIR_APPLICATION == DIR_REAMUR . 'admin/') {
			$data['admin'] = 'admin';
		} else {
			$data['admin'] = '';
		}
	...
	/**
	 * Get document root with fallback options
	 * @return string
	 */
	private function getDocumentRoot(): string {
		// Try $_SERVER['DOCUMENT_ROOT'] first
		if (isset($this->request->server['DOCUMENT_ROOT']) && !empty($this->request->server['DOCUMENT_ROOT'])) {
			return $this->request->server['DOCUMENT_ROOT'];
		}
		
		// Fallback 1: Use SCRIPT_FILENAME
		if (isset($this->request->server['SCRIPT_FILENAME'])) {
			return dirname($this->request->server['SCRIPT_FILENAME']);
		}
		
		// Fallback 2: Use __FILE__ to determine web root
		// Assuming this file is in admin/controller/common/ relative to web root
		return dirname(dirname(dirname(dirname(__FILE__))));
		
		// Fallback 3: Use DIR_REAMUR as last resort
		return rtrim(DIR_REAMUR, '/');
	}
	...
	public function storage(): void {
		...
		if ($this->user->hasPermission('modify', 'common/security')) {
			...
			// Fix: Handle missing DOCUMENT_ROOT
			$document_root = $this->getDocumentRoot();
			$root = str_replace('\\', '/', realpath($document_root . '/../'));
			...
			if (!is_writable(DIR_REAMUR . 'config.php') || !is_writable(DIR_APPLICATION . 'config.php')) {
	...
	public function admin(): void {
		...
		if ($this->user->hasPermission('modify', 'common/security')) {
			$base_old = DIR_REAMUR . 'admin/';
			$base_new = DIR_REAMUR . $name . '/';
			...
			if (($page * $limit) <= $total) {
				...
				foreach ($lines as $line_id => $line) {
					...
					if (strpos($line, 'define(\'DIR_APPLICATION') !== false) {
						$output .= 'define(\'DIR_APPLICATION\', DIR_REAMUR . \'' . $name . '/\');' . "\n";
