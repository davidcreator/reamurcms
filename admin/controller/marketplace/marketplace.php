<?php
namespace Reamur\Admin\Controller\Marketplace;
/**
 * Class Marketplace
 *
 * @package Reamur\Admin\Controller\Marketplace
 */
class Marketplace extends \Reamur\System\Engine\Controller {
	/**
	 * @return void
	 */
	public function index(): void {
		...
		$string .= $this->config->get('reamur_username') . "\n";
		$string .= $this->request->server['HTTP_HOST'] . "\n";
		$string .= VERSION . "\n";
		$string .= $time . "\n";

		$signature = base64_encode(hash_hmac('sha1', $string, $this->config->get('reamur_secret'), 1));

		$url  = '&username=' . urlencode($this->config->get('reamur_username'));
		...
		$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/marketplace' . $url);
		...
		$data['signature'] = $this->config->get('reamur_username') && $this->config->get('reamur_secret');

		if (!$this->config->get('reamur_username') || !$this->config->get('reamur_secret')) {
	...
	public function info(): object|null {
		...
		$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/marketplace/info' . $url);
		...
		if ($response_info) {
			...
			$this->document->addStyle('view/js/jquery/magnific/magnific-popup.css');
			$this->document->addScript('view/js/jquery/magnific/jquery.magnific-popup.min.js');
			...
			return new \Reamur\System\Engine\Action('error/not_found');
		}
	}
	...
	public function purchase(): void {
		...
		if (!$json) {
			...
			$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/marketplace/purchase' . $url);
	...
	public function download(): void {
		...
		if (!$json) {
			...
			$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/marketplace/download&extension_download_id=' . $extension_download_id . $url);
			...
			if (isset($response_info['download'])) {
				if (substr($response_info['filename'], -10) == '.ocmod.zip') {
					...
					$extension_data = [
						...
						'link' 					=> REAMUR_SERVER . 'index.php?route=marketplace/extension.info&extension_id=' . $extension_id
					];
	...
	public function addComment(): void {
		...
		if (!$this->config->get('reamur_username') || !$this->config->get('reamur_secret')) {
			$json['error'] = $this->language->get('error_reamur');
		}
		...
		if (!$json) {
			...
			$string .= urlencode($this->config->get('reamur_username')) . "\n";
			$string .= $this->request->server['HTTP_HOST'] . "\n";
			...
			$url  = '&username=' . $this->config->get('reamur_username');
			$url .= '&domain=' . $this->request->server['HTTP_HOST'];
			$url .= '&version=' . VERSION;
			$url .= '&extension_id=' . $extension_id;
			...
			$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/marketplace/addcomment&extension_id=' . $extension_id . $url);
	...
	public function comment(): void {
		...
		$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/marketplace/comment&extension_id=' . $extension_id . '&page=' . $page);
	...
	public function reply(): void {
		...
		$curl = curl_init(REAMUR_SERVER . 'index.php?route=api/marketplace/comment&extension_id=' . $extension_id . '&parent_id=' . $parent_id . '&page=' . $page);
