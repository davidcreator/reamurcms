 ...
 */
class UserPermission extends \Reamur\System\Engine\Controller {
	/**
	 * @return void
	 */
	...
	public function form(): void {
		...
		foreach ($results as $result) {
			$extension_permission = 'extension/' . str_replace('admin/controller/', '', substr($result['path'], 0, strrpos($result['path'], '.')));
			$data['extensions'][] = $extension_permission;
		}

		// Remove duplicates and sort extensions
		$data['extensions'] = array_unique($data['extensions']);
		sort($data['extensions']);

		if (isset($user_group_info['permission']['access']) && is_array($user_group_info['permission']['access'])) {
			$data['access'] = array_unique($user_group_info['permission']['access']);
		} else {
			$data['access'] = [];
		}

		if (isset($user_group_info['permission']['modify']) && is_array($user_group_info['permission']['modify'])) {
			$data['modify'] = array_unique($user_group_info['permission']['modify']);
		} else {
			$data['modify'] = [];
		}
	...
	public function save(): void {
		...
		if (!$json) {
			...
			// Sanitize and deduplicate permissions before saving
			$post_data = $this->request->post;
			
			if (isset($post_data['permission']['access']) && is_array($post_data['permission']['access'])) {
				$post_data['permission']['access'] = array_unique(array_filter($post_data['permission']['access']));
			} else {
				$post_data['permission']['access'] = [];
			}

			if (isset($post_data['permission']['modify']) && is_array($post_data['permission']['modify'])) {
				$post_data['permission']['modify'] = array_unique(array_filter($post_data['permission']['modify']));
			} else {
				$post_data['permission']['modify'] = [];
			}

			if (!isset($this->request->post['user_group_id']) || !$this->request->post['user_group_id']) {
				$json['user_group_id'] = $this->model_user_user_group->addUserGroup($post_data);
			} else {
				$this->model_user_user_group->editUserGroup($this->request->post['user_group_id'], $post_data);
			}
